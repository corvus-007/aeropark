...
| `button.action` | `show-popup-rent-store`                                                         | Действие кнопки. Действие будет записано в data-атрибут `data-map-place-action="{action}"`. Далее нужно будет написать функию-обработчик при клике на кнопку с `data-map-action` и в зависимости от значения атрибута выполнить определенную функцию. |
