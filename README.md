...
| `for-who`*      | ```new Set([forWhoFilter[`Мужская одежда`], forWhoFilter[`Женская одежда`]])``` | Целевая аудитория магазина. Целевая аудитория находится в файле `filter-options.js`. **\*Предполагается переименовать ключ в `audience` или `target-audience`, вместо `for-who`.**                                                              |
